...
export default function Contact() {
  ...
  return (
    <section id="contact" style={{ background: "#141414", padding: "6rem 0", position: "relative", overflow: "hidden" }}>
      <div className="orb" style={{ width: 500, height: 500, bottom: -200, left: -100, background: "radial-gradient(circle, rgba(172,137,255,0.10) 0%, transparent 70%)" }} />
      <div className="orb" style={{ width: 400, height: 400, top: -100, right: -100, background: "radial-gradient(circle, rgba(143,245,255,0.08) 0%, transparent 70%)" }} />

      <div className="container">
        <div className="contact-grid">
          {/* Left */}
          <div>
            <p className="label-caps" style={{ color: "#8ff5ff", marginBottom: "1.25rem" }}>Связаться</p>
            <h2 className="display" style={{ fontSize: "clamp(2rem, 4vw, 3rem)", lineHeight: 1.1, marginBottom: "1.5rem" }}>
              ГОТОВЫ К<br /><span className="text-gradient-primary">ПРОРЫВУ</span>?
            </h2>
            ...
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.875rem" }}>
              {["Стратегия из медиа-плана", "Анализ текущей ситуации"].map((item, i) => (
                <li key={i} style={{ display: "flex", alignItems: "center", gap: "0.75rem", fontSize: "0.875rem", color: "#adaaaa" }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#8ff5ff", flexShrink: 0 }} />
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div
            style={{
              background: "#1a1919", borderRadius: "1.25rem", padding: "2.5rem",
              border: "1px solid rgba(73,72,71,0.2)",
            }}
          >
            {submitted ? (
              <div style={{ textAlign: "center", padding: "2rem 0" }}>
                <p style={{ fontFamily: "var(--font-space-grotesk), sans-serif", fontSize: "1.5rem", fontWeight: 700, color: "#8ff5ff", marginBottom: "1rem" }}>
                  Заявка отправлена!
                </p>
